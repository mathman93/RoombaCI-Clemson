...
def checkMic(micStatus, micNum):
    global notHeards
    if notHeards[micNum-1] and micStatus==1:
        global lastHeard
        lastHeard=time.time()
        notHeards[micNum-1]=False
        #notHeard=False
        #micTime=time.time()
        #recentHeard=micTime
...
timeBase=time.time()
GPIO.output(reset, GPIO.HIGH)
one=threading.Thread(target=checkMic, args=(statusOne, oneNotHeard), daemon=False)
two=threading.Thread(target=checkMic, args=(statusTwo, twoNotHeard), daemon=False)
three=threading.Thread(target=checkMic, args=(statusThree, threeNotHeard), daemon=False)
threads=[]
threads.append(one)
threads.append(two)
