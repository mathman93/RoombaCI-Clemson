...
timeBase=time.time()
GPIO.output(reset, GPIO.HIGH)
one=threading.Thread(target=checkMic, args=(micOne, 1), daemon=False)
two=threading.Thread(target=checkMic, args=(micTwo, 2), daemon=False)
three=threading.Thread(target=checkMic, args=(micThree, 3), daemon=False)
threads=[]
threads.append(one)
threads.append(two)
