...
''' Returns necessary change in heading when a sync_pulse is received
	StaleDesync calculates midpoint between front and back neighbors
	'''
...
while True:
	try:
		...
		if (angle + counter) > cycle_threshold: # If (angle + counter) is greater than 360 degrees...
			SendSyncPulse()
			pulseCounter = 0 # reset pulseCounter to 0 so that next received pulse is known as an 'after' pulse
			counter_base += counter_adjust
